...
app.delete("/delete-yarn/:id", async (req, res) => {
    ...
    try {
        ...
        const projectUpdatePromises = yarnData.usedInProjects.map(async (projectEntry) => {
        ...
        res.status(200).json({ message: "Yarn and related project updates completed" });
...
app.delete("/delete-project/:id", async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ message: "Project and related yarn updates completed" });

    } catch (error) {
        console.error("Error deleting project or image:", error);
        res.status(500).json({ error: "Failed to delete project and update yarn references" });
    }
});
